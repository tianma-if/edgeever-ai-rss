    ...
    try {
      ...
      const ai = await context.ai.generate({
        system: [
          ...
          "根据候选文章输出版式精致、层级清晰的 Markdown 日报正文。不要输出一级标题，不要输出来源汇总列表。",
          "正文第一行必须输出一段简明精炼的「今日速览」（以“> 💡 **今日速览**：”开头，用 2 至 3 句话综合概括今日最重要的宏观行业趋势或技术动态），随后输出一条分割线“---”。",
          "按重要性输出 7 至 10 个互不重复的热点。每个热点只使用一个二级标题，按“## 01 | 热点概括”至“## 10 | 热点概括”的格式顺序编号（两位数补零），其中“热点概括”必须是该事件具体、准确且信息密度高的标题。",
