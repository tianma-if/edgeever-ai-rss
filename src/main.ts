    ...
    try {
      ...
      const ai = await context.ai.generate({
        system: [
          ...
          "按重要性输出 7 至 10 个互不重复的热点。每个热点只使用一个二级标题，按“## 1️⃣ 热点概括”至“## 🔟 热点概括”的格式顺序编号，其中“热点概括”必须是该事件具体、准确且信息密度高的标题。",
          "禁止使用“热点一”“热点二”等泛化标题，也不要在标题下重复书写粗体标题。每个热点写 2 至 3 条简短要点，不要使用其他标题。",
