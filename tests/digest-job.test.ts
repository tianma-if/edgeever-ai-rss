...
import { runCategoryDigestJob } from "../src/main";
...
const contextWith = (generate: PluginContext["ai"]["generate"], autoTranslate = false): PluginContext => ({
  ...
  notes: {
    ...
    create: async () => { throw new Error("disk is full"); },
...
describe("digest job failures", () => {
  ...
  test("keeps translation requests within the host output token limit", async () => {
    ...
    expect(limits).toEqual([5_000, 4_000]);
  });
});
